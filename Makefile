...
$(LUAROCKS_DEPS): $(LUAROCKS) $(BUSTED_HTEST) $(NLUA)
	...
	@$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
	luarocks make ulf.testmod1-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
	luarocks test --prepare ulf.testmod1-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
